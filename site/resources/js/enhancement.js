...
(function() {
  ...
  function setValue(rating, value) {
    ...
    input.value = value;
    Array.prototype.slice.call(rating.querySelectorAll('.spectrum-Rating-icon')).forEach(function(el, index) {
  ...
  document.addEventListener('click', function(event) {
    ...
    if (icon) {
      var rating = event.target.closest('.spectrum-Rating');
      var value = Array.prototype.slice.call(rating.querySelectorAll('.spectrum-Rating-icon')).indexOf(icon) + 1;
      setValue(rating, value);
    }
  });
}());
